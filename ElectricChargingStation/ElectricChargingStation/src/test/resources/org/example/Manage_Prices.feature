Feature: Manage Prices
  ...
  Scenario: Ladepunktpreis festlegen (AC/DC)
    ...
    When I set the charging point price for "AC_1" at location "Deutschwagram" to AC price per kWh "0.42" and AC price per minute "1.0"
    Then the charging point "AC_1" at location "Deutschwagram" has AC price per kWh "0.42"
    And the charging point "AC_1" at location "Deutschwagram" has AC price per minute "1.0"
    And the charging point price is saved successfully

  Scenario: Standortpreis festlegen (AC/DC)
    Given I am logged in as admin
    And a location with name "Deutschwagram" exists
    When I set the location price for "Deutschwagram" to AC kWh "0.42", AC per minute "1.0", DC kWh "0.55", and DC per minute "3.0"
    Then the location "Deutschwagram" has AC price per kWh "0.42"
    And the location "Deutschwagram" has AC price per minute "1.0"
    And the location "Deutschwagram" has DC price per kWh "0.55"
    And the location "Deutschwagram" has DC price per minute "3.0"
